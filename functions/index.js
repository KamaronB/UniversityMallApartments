...
/*  //exports.BrainTree = functions.https.onRequest((request, response) => {
...
exports.StripeCharge = functions.database.ref(`/stripeTokens/{userId}/Your Token/{id}`)
.onWrite((change,context) => {
	...
	const userId = change.after.userId;
...
return event.data() 
		...
		.then(charge =>{
			...
			admin.database()
				.ref(`/stripeTokens/{userId}/Your Token/charge`)
				.set(charge)
				return;
		})
...
exports.addPaymentSource = functions.database
    .ref(`/stripeToken/{uid}/Your Token/card/id`).onWrite((change, context) => {
...
exports.cleanupUser = functions.auth.user().onDelete((user) => {
  return admin.database().ref(`/stripe_customers/${user.uid}`).once('id').then(
      (snapshot) => {
        return snapshot.val();
      }).then((customer) => {
